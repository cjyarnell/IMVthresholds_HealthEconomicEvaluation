...
psa <- read_csv("PSA_primary.csv", skip = 2)
...
bind_rows(tbl2, icur) %>% write_csv("table2_simple.csv")

# CEAC

wtp_thresholds = seq(from = 0, to = 200000, by = 1000)

CEAC_simple(psa, cost_columns, qaly_columns, wtp_thresholds) 

ggsave("figures/FigureCEAC_simple.svg", width = 7, height = 5)

# joint CEAC

cost_qaly_columns = 1:5

psa1 <- read_csv("PSA_simple.csv",skip = 2)[,cost_qaly_columns]
psa2 <- read_csv("PSA_sensitivity.csv",skip = 2)[,cost_qaly_columns]

names(psa1) <- c("Iteration","Cost_UC","Cost_Thresh","QALY_UC","QALY_Thresh")
names(psa2) <- names(psa1)

psa_joint <- bind_rows(S1 = psa1,S2 = psa2,
                       .id = "source")

jointCEAC_simple(psa_joint, wtp_thresholds) 

ggsave("figures/Figure1jointCEAC.svg", height = 8, width = 6)
ggsave("figures/Figure1jointCEAC.pdf", height = 8, width = 6)
